...
/*
output 包用于处理FuzzGIU的输出结果，主要提供3类函数：Init、Output和Finish。
	InitOutput函数必须在单协程中调用，不会自动检查协程安全问题
	Output类函数用于将结果输出到指定文件或输出流，需要在多协程的环境下调用，因此内部检查协程安全
	Finish类函数用于结束输出，具体结束的结果取决于内部实现，这个函数同样需要在单协程环境中调用
*/
...
func Output(obj *OutObj, toWhere int32) {
	if toWhere&OutToFile != 0 {
		fo.Output((*common.OutObj)(obj))
	}
	if toWhere&OutToScreen != 0 {
		so.Output((*common.OutObj)(obj))
	}
	if toWhere&OutToNativeStdout != 0 {
		native.Output((*common.OutObj)(obj))
	}
}
