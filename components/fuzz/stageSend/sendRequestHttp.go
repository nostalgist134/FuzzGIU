...
func initHttpCli(proxy string, timeout int, redirect bool, httpVer string, redirectChain *string) (*http.Client, error) {
	cli := cliPool.Get().(*http.Client) // 从池中获取一个 http.Client
...
func fuzzReq2HttpReq(fuzzReq *fuzzTypes.Req) (*http.Request, error) {
	...
	req, err = http.NewRequest(fuzzReq.HttpSpec.Method, URL, bytes.NewBuffer([]byte(fuzzReq.Data)))
	if err != nil {
		return nil, err
	}
	// proto，即HTTP/1.1部分
	req.Proto = fuzzReq.HttpSpec.Version
	// http请求头部分
	for i := 0; i < len(fuzzReq.HttpSpec.Headers); i++ {
		...
		if indColon == len(fuzzReq.HttpSpec.Headers[i])-1 || indColon == -1 {
			req.Header.Add(fuzzReq.HttpSpec.Headers[i], "")
		} else {
			req.Header.Add(fuzzReq.HttpSpec.Headers[i][0:indColon],
...
func sendRequestHttp(request *fuzzTypes.Req, timeout int, httpRedirect bool, retry int,
	...
	if sendErr == nil {
		...
		if buildErr != nil {
			fmt.Printf("sendRequestHttp Failed - Cannot get raw http response: %v\n", buildErr)
			rawResp = nil
	...
	containRetryCode := func(code int, retryCodes []string) bool {
	...
	if (retryRegex != "" && common.RegexMatch(rawResp, retryRegex)) ||
		...
		for ; retry > 0; retry-- {
			...
			// patchLog#2: 重新填充data部分，这是必须的，因为http.request每次发完请求后就会把body消耗掉（完全傻逼的设计）
			req.Body = io.NopCloser(bytes.NewBuffer(reqData))
			// 重试请求
			httpResponse, sendErr = cli.Do(req)
			resp.HttpResponse = httpResponse
			resp.RespError = sendErr // 每次发送请求后都设置respError位
			rawResp, buildErr = buildRawHTTPResponse(httpResponse)
			if buildErr != nil {
				fmt.Printf("sendRequestHttp Failed - Cannot get raw http response: %v\n", buildErr)
				rawResp = nil
