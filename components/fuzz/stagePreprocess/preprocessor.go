...
func Preprocess(fuzz *fuzzTypes.Fuzz, preprocessors []fuzzTypes.Plugin, outCtx *output.Ctx) *fuzzTypes.Fuzz {
	...
	for keyword, _ := range newFuzz.Preprocess.PlTemp {
		...
		if len(plList) == 0 {
			plList = PayloadGenerator(newFuzz.Preprocess.PlTemp[keyword].Generators, outCtx)
		}
