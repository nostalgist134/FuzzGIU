	...
	for i := int64(0); i < plListsLen; i++ {
		...
		if fuzz.Preprocess.Mode != "sniper" && fuzz.React.RecursionControl.MaxRecursionDepth <= 0 {
			...
			task = func() *fuzzTypes.Reaction {
				...
				return stageReact.React(fuzz, sendMeta.Request, resp, keywords, payloadEachKeyword, nil)
			}
		} else { // sniper模式或者递归模式
			keyword := keywords[0]
			...
			task = func() *fuzzTypes.Reaction {
				resp := stageSend.SendRequest(sendMeta)
				return stageReact.React(fuzz, sendMeta.Request, resp, []string{keyword}, []string{payload}, recursionPos)
			}
		}
		/*------------------------------------ 发送阶段 ------------------------------------*/
		Wp.Submit(task)
