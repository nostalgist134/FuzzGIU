	...
	for _, tmp := range pluginStrs {
		...
		if appendType == appendGen {
			...
			tempMap[keyword] = fuzzTypes.PayloadTemp{
				Generators: pluginExpression + suffix + originalGenerators, // 如果键已经存在，则拼接generator+","+原来的generator
...
// parseCommaSeparatedStr 将形如1,2,3,114514的字符串转化为int型切片
func parseCommaSeparatedStr(str string) []int {
	if str == "" {
		return nil
	}
	strSplit := strings.Split(str, ",")
	ret := make([]int, len(strSplit))
...
func opt2fuzz(opt *options.Opt) *fuzzTypes.Fuzz {
	...
	fuzz.React.IgnoreError = opt.General.IgnoreError
	// opt.HTTP
	...
	fuzz.React.Filter.Time.UpBound = time.Duration(upbound) * time.Second
	fuzz.React.Filter.Time.DownBound = time.Duration(downbound) * time.Second
	...
	fuzz.React.Matcher.Time.UpBound = time.Duration(upbound) * time.Second
	fuzz.React.Matcher.Time.DownBound = time.Duration(downbound) * time.Second
	fuzz.React.Matcher.Mode = opt.Matcher.MatcherMode
	// opt.Output
	fuzz.React.Verbosity = opt.Output.Verbosity
	// opt.ErrorHandling
	...
	*/
	...
	for i, preprocessors := range opt.Plugin.Preprocessors {
		strBuilder.WriteString(preprocessors)
		if i != len(opt.Plugin.Preprocessors)-1 {
			strBuilder.WriteString(",")
		}
	}
	fuzz.Preprocess.Preprocessors = strBuilder.String()
	...
	if opt.RecursionControl.Recursion {
		...
		for k, _ := range fuzz.Preprocess.PlTemp {
			// 递归关键字设置为从关键字列表中取的第一个键（如果只有一个关键词的话就无所谓，如果指定多个关键字就不管了）
